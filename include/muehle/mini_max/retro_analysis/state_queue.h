...
class StateQueue {
 public:
  ...
  long long GetNumStatesToProcess() { return num_states_to_process; }
  PlyInfoVarType GetMaxPlyInfoValue() { return max_ply_info_value; }

 private:
  Logger& log; /* Logger, used for output */
  std::vector<CyclicArray*>
      states_to_process; /* Cyclic array containing the states, whose short know
                            values are known for sure. They have to be processed
                          */

  long long num_states_to_process =
      0; /* Number of states in 'states_to_process' which have to be processed
          */
  PlyInfoVarType max_ply_info_value = 0; /* Maximum ply info value */
  std::wstring file_directory;        /* Directory where the files are stored */
  unsigned int thread_no = 0xffff;    /* Thread number, used for file names */
