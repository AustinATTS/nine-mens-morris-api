...
/* Each thread gets the same number of iterations */
...
/* Number of scheduling types */
#define TM_SCHEDULE_NUM_TYPES 5

/* Return values of the execution functions */
...
class ThreadManagerClass {
 private:
  ...
  struct ForLoopStruct {
    /* Type of scheduling, for load balancing */
    unsigned int schedule_type = TM_SCHEDULE_USER_DEFINED;
    ...
    /* Final value of the loop */
    int64_t final_value = 0;

    /* Pointer to the user defined structure */
    void* p_parameter = nullptr;

    /* Pointer to the user function to be executed by the threads */
    DWORD (*thread_proc)(void* p_parameter, int64_t index) = nullptr;

    /* Pointer to the ThreadManagerClass object */
    ThreadManagerClass* thread_manager;
  };

  struct ThreadItem {
    /* The thread handle given by the system */
    HANDLE h_thread;

    /* The thread ID given by the system */
    DWORD thread_id;

    /* The thread number from 0 to num_threads-1 */
  ...
  /* True when CancelExecution() was called */
  bool terminate_all_threads = false;

  /* True if thread execution is currently paused */
  bool execution_paused = false;

  /* True when CancelExecution() was called */
  bool execution_cancelled = false;

  /* Array of size 'num_threads' containing the thread handles, thread ids */
  std::vector<ThreadItem> threads;

  /* Pointer to a barrier object */
  std::barrier<>* p_barrier = nullptr;

  /* True if any thread is on the last iteration */
  bool any_thread_on_last_iteration = false;
 ...
 public:
  ...
  class ThreadVarsArray {
   public:
    ...
    /* Array of size 'number_of_threads' containing the ThreadVarsArrayItem
     * objects */
    std::vector<VarType> item;
  ...
  /* Returns a number from 0 to 'num_threads'-1 */
  unsigned int GetThreadNumber();

  /* Returns the total number of threads */
  unsigned int GetNumThreads();

  /* Tries to set the number of threads. Returns false if any thread is running.
   */
  bool SetNumThreads(unsigned int new_num_threads);

  /* Waits for all threads to reach this point */
  ...
  /* A user function which is called every x-milliseconds during execution
   * between two iterations */
  void SetCallBackFunction(void UserFunction(void* p_user), void* p_user,
                           DWORD milliseconds);
