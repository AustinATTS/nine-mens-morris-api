...
class FieldStructTypes {
 public:
  /* Constants */
  static const unsigned int max_num_pos_moves =
      3 * 18 *
      9; /* 3 stones can be moved to 18 positions, 9 stones can be removed */
  static const PlayerId player_black =
      PlayerId::player_one; /* Define player one as black (PlayerId::player_one
                               is used for black stones) */
  static const PlayerId player_white =
      PlayerId::player_two; /* Define player two as white (PlayerId::player_two
                               is used for white stones) */
  static const unsigned int num_stones_per_player =
      9;                           /* Number of stones per player */
  static const int size = 24;      /* Number of squares */
  static const int game_drawn = 3; /* Only a nonzero value */

  /* typedef */
  using FieldPos =
      unsigned int; /* Type representing the position of a stone on the field */
  using FieldArray =
      std::array<PlayerId,
                 size>; /* Type representing the field as an array of PlayerIds,
                           indicating the stone on each field position */
  using MillArray =
      std::array<unsigned int,
                 size>; /* Type representing the mills as an array of unsigned
  ...
  class Core {
   public:
    /* Core variables */
    FieldArray field; /* One of the values above for each field position,
                         initialised with square_is_free */
    bool setting_phase = true;     /* True if stones_set < 18 */
    PlayerStruct::Core cur_player; /* Pointers to the current player */
