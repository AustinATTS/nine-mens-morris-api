...
bool compressor::File::SectionInfo::Write(std::fstream& fs,
  ...
  /* Section keys are always serialized on disk as 16-bit code units
   * (char16_t), NOT the platform's native wchar_t. wchar_t is 2 bytes on
   * Windows/MSVC (UTF-16) but 4 bytes on Linux/macOS/GCC (UCS-4), so writing
   * raw wchar_t bytes makes database files unreadable across platforms --
   * this is what caused "Database reports 0 layers" when opening a
   * Windows-generated database on Linux. char16_t is fixed at 16 bits on
   * every conforming platform and matches Windows' native wchar_t exactly,
   * so this format is portable both ways. Keys here are always plain
   * ASCII identifiers (e.g. "dbStats", "layerStats0.succLayers"), so the
...
bool compressor::File::SectionInfo::Read(std::fstream& fs,
  ...
  /* locals: read on-disk key names as 16-bit code units (char16_t), matching
   * the fixed portable on-disk format written above. NOT the platform's
