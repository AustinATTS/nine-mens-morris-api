...
/* Destructor */
mini_max::database::UncompFile::~UncompFile() {
  CloseDatabase();
}

/* Opens the database allowing read and write operations.  Fails if the database
 * is already open. */
bool mini_max::database::UncompFile::OpenDatabase(
  ...
  if (IsOpen()) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"Database is already open.");
  }
  ...
  if (h_file_short_knot_values == INVALID_HANDLE_VALUE) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"Failed to open short knot value file.");
  }
  ...
  /* FILE_FLAG_RANDOM_ACCESS) */
  h_file_ply_info =
      CreateFile(/* path: */ ss_file.str().c_str(),
                 /* access: */ GENERIC_READ | GENERIC_WRITE,
                 FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
                 /* creation: */ OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  ...
  if (h_file_ply_info == INVALID_HANDLE_VALUE) {
    ...
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"Failed to open ply info file.");
  }

  return true;
}

/* Removes the database files from the disk.  Close the database, if necessary,
 * before deletion. */
bool mini_max::database::UncompFile::RemoveFile(
  ...
  ss_database_file.str(/* s: */ L"");
  ss_database_file << file_directory << (file_directory.size() ? "/" : "")
                   << "short_knot_value.dat";
  if (GetFileAttributes(/* path: */ ss_database_file.str().c_str()) !=
      INVALID_FILE_ATTRIBUTES) {
    _wremove(/* path: */ ss_database_file.str().c_str());
  }
  ...
  if (GetFileAttributes(/* path: */ ss_database_file.str().c_str()) !=
      INVALID_FILE_ATTRIBUTES) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"Failed to delete database files.");
  }
...
/* Loads the header and stats from the database files into memory.  New files
 * are created if they do not exist.  The header is checked for consistency with
 * the loaded files.  db_stats and layer_stats will be completely overwritten.
 * Returns false if the header is not loaded.  db_stats: 	 general information
 * about the database  layer_stats: layer specific information */
bool mini_max::database::UncompFile::LoadHeader(
    DatabaseStatsStruct& db_stats, std::vector<LayerStatsStruct>& layer_stats) {
  if (!IsOpen()) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"Cannot load header, since database is not open.");
  }
  if (!OpenSkvFile(db_stats, layer_stats)) {
    return false;
  }
  if (!OpenPlyInfoFile(layer_stats)) {
    return false;
  }
...
/* Saves the header and stats to the database files.  They are NOT checked for
 * consistency with the written skv and  plyInfo. */
bool mini_max::database::UncompFile::SaveHeader(
  ...
  if (!IsOpen()) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"Cannot save header, since database is not open.");
  }
  ...
  if (!SaveSkvHeader(skvf_header, my_layer_stats)) {
    return false;
  }
  if (!SavePlyHeader(ply_info_header, ply_infos)) {
    return false;
  }
  return true;
}

/* Returns true if the handles to both database files are not NULL. */
...
/* Read all short knot values from the database for a given layer.  The passed
 * std::vector must have the correct size  myLayerStats[layerNum].sizeInBytes,
 * which can be 0. */
bool mini_max::database::UncompFile::ReadSkv(unsigned int layer_num,
  ...
  if (layer_num >= my_layer_stats.size()) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"readSkv() failed. Layer number out of range.");
  }
  ...
  if (h_file_short_knot_values == INVALID_HANDLE_VALUE) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readSkv() failed. Database file not open.");
  }
  if (skvf_header.header_and_stats_size == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readSkv() failed. Header not loaded.");
  }
  if (my_layer_stats[layer_num].size_in_bytes == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readSkv() failed. Layer has no knots.");
  }
  if (!my_layer_stats[layer_num].layer_is_completed_and_in_file) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readSkv() failed. Layer is not in file.");
  }
  return LoadBytesFromFile(h_file_short_knot_values,
                           /* offset: */ skvf_header.header_and_stats_size +
                               my_layer_stats[layer_num].layer_offset,
                           my_layer_stats[layer_num].size_in_bytes, &skv[0]);
}

/* Read a short knot value from the database for a given layer and state
  number. */
bool mini_max::database::UncompFile::ReadSkv(unsigned int layer_num,
  ...
  if (state_number >= my_layer_stats[layer_num].knots_in_layer) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"readSkv() failed. State number out of range.");
  }
  if (my_layer_stats[layer_num].size_in_bytes == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readSkv() failed. Layer has no knots.");
  ...
  return LoadBytesFromFile(h_file_short_knot_values,
                           /* offset: */ skvf_header.header_and_stats_size +
                               my_layer_stats[layer_num].layer_offset +
                               state_number / 4,
                           /* num_bytes: */ sizeof(TwoBit), &database_byte);
}

/* Write all short knot values to the database for a given layer.  The passed
 * std::vector must have the correct size  myLayerStats[layerNum].sizeInBytes,
 * which can be 0. */
bool mini_max::database::UncompFile::WriteSkv(unsigned int layer_num,
  ...
  if (layer_num >= my_layer_stats.size()) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"writeSkv() failed. Layer number out of range.");
  }
  if (skv.size() != my_layer_stats[layer_num].size_in_bytes) {
    return log.Log(
        Logger::LogLevel::error,
  ...
  if (h_file_short_knot_values == INVALID_HANDLE_VALUE) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"writeSkv() failed. Database file not open.");
  }
  if (skvf_header.header_and_stats_size == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"writeSkv() failed. Header not loaded.");
  }
  if (my_layer_stats[layer_num].size_in_bytes == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"writeSkv() failed. Layer has no knots.");
  }
...
/* Read all ply information from the database for a given layer.  The passed
 * std::vector must have the correct size  plyInfos[layerNum].sizeInBytes, which
 * can be 0. */
bool mini_max::database::UncompFile::ReadPlyInfo(
  ...
  if (layer_num >= ply_infos.size()) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"readPlyInfo() failed. Layer number out of range.");
  }
  ...
  if (h_file_ply_info == INVALID_HANDLE_VALUE) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"readPlyInfo() failed. Database file not open.");
  }
  if (ply_info_header.header_and_ply_infos_size == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readPlyInfo() failed. Header not loaded.");
  }
  if (ply_infos[layer_num].size_in_bytes == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readPlyInfo() failed. Layer has no knots.");
  }
  if (!ply_infos[layer_num].ply_info_is_completed_and_in_file) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"readPlyInfo() failed. Layer is not in file.");
  }
  return LoadBytesFromFile(
      h_file_ply_info,
      /* offset: */ ply_info_header.header_and_ply_infos_size +
          ply_infos[layer_num].layer_offset,
      ply_infos[layer_num].size_in_bytes, &ply_info[0]);
}

/* Read the ply information from the database for a given layer and state
  number. */
bool mini_max::database::UncompFile::ReadPlyInfo(
  ...
  if (state_number >= ply_infos[layer_num].knots_in_layer) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"readPlyInfo() failed. State number out of range.");
  }
  if (ply_infos[layer_num].size_in_bytes == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"readPlyInfo() failed. Layer has no knots.");
  ...
  return LoadBytesFromFile(
      ...
      /* num_bytes: */ sizeof(PlyInfoVarType), &single_ply_info);
}

/* Write all ply information to the database for a given layer. */
bool mini_max::database::UncompFile::WritePlyInfo(
    unsigned int layer_num, const std::vector<PlyInfoVarType>& ply_info) {
  if (layer_num >= ply_infos.size()) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"writePlyInfo() failed. Layer number out of range.");
  }
  if (ply_info.size() != ply_infos[layer_num].knots_in_layer) {
    return log.Log(
        Logger::LogLevel::error,
  ...
  if (h_file_ply_info == INVALID_HANDLE_VALUE) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"writePlyInfo() failed. Database file not open.");
  }
  if (ply_info_header.header_and_ply_infos_size == 0) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"writePlyInfo() failed. Header not loaded.");
  }
  if (ply_infos[layer_num].size_in_bytes == 0) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"writePlyInfo() failed. Layer has no knots.");
  }
...
/* Close the database files.  The data in the memory is not affected. */
...
/* Write a number of bytes to a file at a given offset.  If operation fails, the
 * function waits for 1 second and tries  again. */
bool mini_max::database::UncompFile::SaveBytesToFile(HANDLE h_file,
  ...
  while (!SetFilePointerEx(h_file, li_distance_to_move, /* new_pos: */ NULL,
    ...
    log.Log(Logger::LogLevel::error,
            std::wstring{/* s: */ L"ERROR: SetFilePointerEx failed!"});
  ...
  while (resting_bytes > 0) {
    if (WriteFile(h_file, my_pointer, resting_bytes, &dw_bytes_written, NULL) ==
      ...
      log.Log(Logger::LogLevel::error,
              std::wstring{/* s: */ L"ERROR: WriteFile Failed!"});
      Sleep(/* milliseconds: */ 1000);
    }
  }
  return true;
...
/* Read a number of bytes from a file at a given offset.  If operation fails,
 * the function waits for 1 second and tries  again. */
bool mini_max::database::UncompFile::LoadBytesFromFile(HANDLE h_file,
  ...
  while (resting_bytes > 0) {
    if (ReadFile(h_file, /* buf: */ p_bytes, resting_bytes, &dw_bytes_read,
                 NULL) == TRUE) {
      ...
      log.Log(Logger::LogLevel::error,
              std::wstring{/* s: */ L"ERROR: ReadFile Failed!"});
      Sleep(/* milliseconds: */ 1000);
    }
  }
  return true;
...
/* Store the header and stats in the short knot value file. */
bool mini_max::database::UncompFile::SaveSkvHeader(
  ...
  if (h_file_short_knot_values == INVALID_HANDLE_VALUE) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"saveSkvHeader() failed. Database file not open.");
  }
  ...
  if (!SaveBytesToFile(h_file_short_knot_values, /* offset: */ 0,
                       /* num_bytes: */ sizeof(SkvFileHeaderStruct), &db_h)) {
    return false;
  }
  for (unsigned int i = 0; i < db_h.num_layers; i++) {
    if (!l_stats[i].SaveToFile(h_file_short_knot_values)) {
      return false;
    }
  }
  return true;
...
/* Store the header and stats in the ply info file. */
bool mini_max::database::UncompFile::SavePlyHeader(
  ...
  if (h_file_ply_info == INVALID_HANDLE_VALUE) {
    return log.Log(
        Logger::LogLevel::error,
        /* message: */ L"savePlyHeader() failed. Database file not open.");
  }
  ...
  if (!SaveBytesToFile(h_file_ply_info, /* offset: */ 0,
                       /* num_bytes: */ sizeof(PlyInfoFileHeaderStruct),
                       &pi_h)) {
    return false;
  }
  if (!SaveBytesToFile(
          h_file_ply_info, /* offset: */ sizeof(PlyInfoFileHeaderStruct),
          /* num_bytes: */ sizeof(PlyInfoFileLayerStruct) * pi_h.num_layers,
          &p_info[0])) {
    return false;
  }
  return true;
...
/* Create an empty short knot value file with the header only. */
...
/* Open the short knot value file and read the header and stats into the
  memory.  If the file is invalid or does not exist, a new file with an  empty
  header is created. */
bool mini_max::database::UncompFile::OpenSkvFile(
  ...
  if (h_file_short_knot_values == INVALID_HANDLE_VALUE) {
    return log.Log(Logger::LogLevel::error,
                   /* message: */ L"Database file not open.");
  }
  ...
  if (file_size < sizeof(SkvFileHeaderStruct)) {
    ...
    /* database complete ? */
    if (!LoadBytesFromFile(h_file_short_knot_values, /* offset: */ 0,
                           /* num_bytes: */ sizeof(SkvFileHeaderStruct),
                           &skvf_header)) {
      return false;
    }
    ...
    if (skvf_header.header_code != SKV_FILE_HEADER_CODE) {
      return log.Log(Logger::LogLevel::error,
                     /* message: */ L"Invalid short knot value file header.");
    }
...
/* Create an empty ply info file with the header only. */
...
/* Open the ply info file and read the header and stats into the memory. If the
  file is invalid or does not exist, a new file with an empty header is created.
*/
bool mini_max::database::UncompFile::OpenPlyInfoFile(
  ...
  if (file_size < sizeof(ply_info_header) + sizeof(PlyInfoFileLayerStruct) *
    ...
    /* database complete ? */
    if (!LoadBytesFromFile(h_file_ply_info, /* offset: */ 0,
                           /* num_bytes: */ sizeof(ply_info_header),
                           &ply_info_header)) {
      return false;
    }
    ...
    if (ply_info_header.header_code != PLYINFO_HEADER_CODE) {
      return log.Log(Logger::LogLevel::error,
                     /* message: */ L"Invalid ply info file header.");
    }

    /* read layer stats */
    ply_infos.resize(ply_info_header.num_layers);
    if (!LoadBytesFromFile(h_file_ply_info,
                           /* offset: */ sizeof(PlyInfoFileHeaderStruct),
                           /* num_bytes: */ sizeof(PlyInfoFileLayerStruct) *
                               ply_info_header.num_layers,
                           /* p_bytes: */ ply_infos.data())) {
      return false;
    }
  }
  return true;
...
bool mini_max::database::UncompFile::SkvFileLayerStruct::LoadFromFile(
    HANDLE h_file) {
  DWORD dw_bytes_read;
  if (!ReadFile(h_file, /* buf: */ &layer_is_completed_and_in_file,
                /* num_bytes_to_read: */ sizeof(layer_is_completed_and_in_file),
                &dw_bytes_read, NULL)) {
    return false;
  }
  if (!ReadFile(h_file, /* buf: */ &layer_offset,
                /* num_bytes_to_read: */ sizeof(layer_offset), &dw_bytes_read,
                NULL)) {
    return false;
  }
  if (!ReadFile(h_file, /* buf: */ &knots_in_layer,
                /* num_bytes_to_read: */ sizeof(knots_in_layer), &dw_bytes_read,
                NULL)) {
    return false;
  }
  if (!ReadFile(h_file, /* buf: */ &num_won_states,
                /* num_bytes_to_read: */ sizeof(num_won_states), &dw_bytes_read,
                NULL)) {
    return false;
  }
  if (!ReadFile(h_file, /* buf: */ &num_lost_states,
                /* num_bytes_to_read: */ sizeof(num_lost_states),
                &dw_bytes_read, NULL)) {
    return false;
  }
  if (!ReadFile(h_file, /* buf: */ &num_drawn_states,
                /* num_bytes_to_read: */ sizeof(num_drawn_states),
                &dw_bytes_read, NULL)) {
    return false;
  }
  if (!ReadFile(h_file, /* buf: */ &num_invalid_states,
                /* num_bytes_to_read: */ sizeof(num_invalid_states),
                &dw_bytes_read, NULL)) {
    return false;
  }
  if (!ReadFile(h_file, /* buf: */ &size_in_bytes,
                /* num_bytes_to_read: */ sizeof(size_in_bytes), &dw_bytes_read,
                NULL)) {
    return false;
  }
  if (!LoadVectorFromFile(h_file, succ_layers)) {
    return false;
  }
  if (!LoadVectorFromFile(h_file, partner_layers)) {
    return false;
  }
  return true;
}
...
bool mini_max::database::UncompFile::SkvFileLayerStruct::SaveToFile(
  ...
  DWORD dw_bytes_written;
  if (!WriteFile(
          h_file, &layer_is_completed_and_in_file,
          /* num_bytes_to_write: */ sizeof(layer_is_completed_and_in_file),
          &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, &layer_offset,
                 /* num_bytes_to_write: */ sizeof(layer_offset),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, &knots_in_layer,
                 /* num_bytes_to_write: */ sizeof(knots_in_layer),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, &num_won_states,
                 /* num_bytes_to_write: */ sizeof(num_won_states),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, &num_lost_states,
                 /* num_bytes_to_write: */ sizeof(num_lost_states),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, &num_drawn_states,
                 /* num_bytes_to_write: */ sizeof(num_drawn_states),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, &num_invalid_states,
                 /* num_bytes_to_write: */ sizeof(num_invalid_states),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, &size_in_bytes,
                 /* num_bytes_to_write: */ sizeof(size_in_bytes),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!SaveVectorToFile(h_file, succ_layers)) {
    return false;
  }
  if (!SaveVectorToFile(h_file, partner_layers)) {
    return false;
  }
  return true;
}

bool mini_max::database::UncompFile::SkvFileLayerStruct::LoadVectorFromFile(
    HANDLE h_file, std::vector<unsigned int>& buffer) {
  DWORD dw_bytes_read;
  unsigned int bytes_to_read;
  if (!ReadFile(h_file, /* buf: */ &bytes_to_read,
                /* num_bytes_to_read: */ sizeof(unsigned int), &dw_bytes_read,
                NULL)) {
    return false;
  }
  buffer.resize(/* new_size: */ bytes_to_read / sizeof(unsigned int));
  return ReadFile(h_file, /* buf: */ buffer.data(), bytes_to_read,
                  &dw_bytes_read, NULL) &&
...
bool mini_max::database::UncompFile::SkvFileLayerStruct::SaveVectorToFile(
  ...
  if (!WriteFile(h_file, &bytes_to_write,
                 /* num_bytes_to_write: */ sizeof(unsigned int),
                 &dw_bytes_written, NULL)) {
    return false;
  }
  if (!WriteFile(h_file, /* buf: */ buffer.data(), bytes_to_write,
                 &dw_bytes_written, NULL)) {
    return false;
  }
