...
bool mini_max::database::CompFile::OpenDatabase(
...
/* Deletes the database file. If 'fileDirectory' is omitted, the function will
 * attempt to remove the file at the current 'fileDirectory' path. */
bool mini_max::database::CompFile::RemoveFile(
    std::wstring const& file_directory) {
...
void mini_max::database::CompFile::UpdateCache(
  ...
  for (size_t i = 0; i < layer_stats.size(); i++) {
    layer_stats_cache[i].completed_and_in_file =
...
bool mini_max::database::CompFile::ReadSection(
    const std::wstring& key, std::vector<unsigned int>& buffer) {
  if (!file_opened) {
...
bool mini_max::database::CompFile::LoadHeader(
  ...
  unsigned int db_stats_compressed_size =
      file.GetSizeOfCompressedSection(db_stats_key);
  ...
  if (file.GetSizeOfCompressedSection(L"dbStats") > 0) {
    if (!file.Read(db_stats_key, 0, sizeof(db_stats), &db_stats)) {
      return log.Log(Logger::LogLevel::error,
                     L"Failed to read dbStats section.");
    }
    ...
    for (unsigned int i = 0; i < db_stats.num_layers; i++) {
      ...
      if (!file.Read(std::wstring(L"layerStats") + std::to_wstring(i), 0,
                     ...
                     &layer_stats[i])) {
        return log.Log(Logger::LogLevel::error,
                       L"Failed to read layerStats section." + base_key_name);
      }
      ...
      if (!ReadSection(base_key_name + std::wstring(L".succLayers"),
        ...
        return log.Log(Logger::LogLevel::error,
                       L"Failed to read succLayers section." + base_key_name +
                           std::wstring(L".succLayers"));
      }
      ...
      if (file.DoesKeyExist(base_key_name + std::wstring(L".partnerLayers"))) {
        if (!ReadSection(base_key_name + std::wstring(L".partnerLayers"),
          ...
          return log.Log(Logger::LogLevel::error,
                         L"Failed to read partnerLayers section." +
                             base_key_name + std::wstring(L".partnerLayers"));
        }
    ...
    log << L"dbStats section was not found. "
        << L"Creating database metadata from game definition.\n";
    ...
    for (unsigned int i = 0; i < db_stats.num_layers; i++) {
      ...
      layer_stats[i].partner_layer =
          layer_stats[i].partner_layers[0]; /* only one partner layer in legacy
                                               databases */
  ...
  for (unsigned int i = 0; i < db_stats.num_layers; i++) {
    ...
    if (layer_stats[i].succ_layers.size()) {
      if (!file.Write(std::wstring(L"layerStats") + std::to_wstring(i) +
                          std::wstring(L".succLayers"),
                      0,
                      sizeof(unsigned int) * layer_stats[i].succ_layers.size(),
    ...
    if (layer_stats[i].partner_layers.size()) {
      if (!file.Write(
              std::wstring(L"layerStats") + std::to_wstring(i) +
                  std::wstring(L".partnerLayers"),
              0, sizeof(unsigned int) * layer_stats[i].partner_layers.size(),
...
bool mini_max::database::CompFile::IsOpen() {
  return file_opened;
}
...
bool mini_max::database::CompFile::ReadPlyInfo(
