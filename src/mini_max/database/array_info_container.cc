...
/* Register a new array in the database
...
bool mini_max::database::ArrayInfoContainer::AddArray(
  ...
  if (num_layers <= layer_number) {
    return false;
  }
  if (type == ArrayInfoStruct::ArrayType::invalid) {
    return false;
  }
  if (type == ArrayInfoStruct::ArrayType::size) {
    return false;
  }
  ...
  if (vector_arrays.size() <= vector_array_index) {
    return false;
  }

  /* If the array already exists, the function returns false */
  if (vector_arrays[vector_array_index] != list_arrays.end()) {
    return false;
  }
...
bool mini_max::database::ArrayInfoContainer::RemoveArray(
  ...
  if (list_arrays.size() == 0) {
    return false;
  }
  if (itr == list_arrays.end()) {
    return false;
  }

  /* does sizes fit? */
  if (itr->belongs_to_layer != layer_number || itr->type != type ||
      itr->size_in_bytes != size ||
      itr->compresed_size_in_bytes != compressed_size) {
    return false;
  }
...
/* Initialize the array info container */
...
/* Return true if there are any array infos to update in the GUI */
bool mini_max::database::ArrayInfoContainer::AnyArrayInfoToUpdate() {
  std::lock_guard<std::mutex> lock(mutex);
  return (array_infos_to_be_updated.size() > 0);
}

/* Get the next array info to update in the GUI */
...
/* Get the index of the array in the vector */
